...
set JAVA_OPTS=-Djava.awt.headless=true -Xverify:none -Dfile.encoding=UTF8 -Dsun.jnu.encoding=UTF-8 -server -Xmx1G -Djava.endorsed.dirs=%DOTCMS_HOME%/WEB-INF/endorsed_libs  -XX:+DisableExplicitGC -javaagent:%DOTCMS_HOME%/WEB-INF/lib/dot.jamm-0.2.5_2.jar
...
if "!SKIP_OPEN_DISTRO!"=="false" (
	...
	for /f %%i in ('%~dp0/curl-7.68.0-win64/bin/curl -s "%ELASTICSEARCH_HOST%:%ELASTICSEARCH_PORT%/_cat/health?h=status" -u %OPEN_DISTRO_USER%:%OPEN_DISTRO_PASSWORD% --insecure') do set health_check=%%i
