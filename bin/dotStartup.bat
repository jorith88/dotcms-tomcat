...
if "!SKIP_OPEN_DISTRO!"=="false" (
	...
	if "%OPEN_DISTRO_ALREADY_RUNNING%"=="false" (
		...
		docker run -d --name dot_opendistro -e PROVIDER_ELASTICSEARCH_HEAP_SIZE=1500m -e PROVIDER_ELASTICSEARCH_DNSNAMES=elasticsearch -e ES_ADMIN_PASSWORD=%OPEN_DISTRO_PASSWORD% -e discovery.type=single-node -p 9200:9200 gcr.io/cicd-246518/es-open-distro:1.3.0
